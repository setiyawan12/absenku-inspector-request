 ...
 */
function createClientServer(localPort) {
  return http.createServer(async (req, res) => {
    const tunnel = state.tunnels.get(localPort);
    if (!tunnel) { res.writeHead(500); res.end('Unknown local port'); return; }
    ...
    if (req.method === 'GET' && req.url === '/connect') {
      ...
      console.log(`✓ Client connected → localhost:${localPort}`);
      sse(res, 'connected', {
      ...
      req.on('close', () => {
        ...
        clearInterval(ping);
        console.log(`✗ Client disconnected → localhost:${localPort}`);
      });
      return;
    }
