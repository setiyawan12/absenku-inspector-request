...
function notify(r){
  ...
  const n=new Notification('WAN NET',{body:\`\${r.method} \${r.url} → \${r.status||'...'}\`});
...
function prependRow(r){
  ...
  document.getElementById('cnt').textContent=
    list.querySelectorAll('.reqrow').length;
  updatePortBar();
}
...
function render(){
  ...
  f.forEach(r=>list.appendChild(makeRow(r,maxMs)));
...
function toggleTheme(){
  ...
  try{window.parent.postMessage({type:'wan-net-theme',theme},'*');}catch{}
}
(function initTheme(){
...
window.addEventListener('message',e=>{
  if(!e.data)return;
  if(e.data.type==='wan-net-theme'){_applyTheme(e.data.theme==='light');return;}
