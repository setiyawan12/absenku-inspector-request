...
<title>absenku Inspector request</title>
<style>
...
<header>
  <div class="logo"><div class="logo-icon">⚡</div>absenku Inspector request</div>
...
function notify(r){
  ...
  const n=new Notification('absenku Inspector request',{body:\`\${r.method} \${r.url} → \${r.status||'...'}\`});
...
function exportHAR(){
  ...
  const blob=new Blob([JSON.stringify({log:{version:'1.2',creator:{name:'absenku Inspector request',version:'1.0'},entries}},null,2)],{type:'application/json'});
