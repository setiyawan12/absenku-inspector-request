...
function createClient() {
  ...
  function connect({ localPort, serverHost = 'localhost', clientPort = 4040, onConnected } = {}) {
    _active     = true;
    _retryDelay = 1000;
    _attempt    = 0;
    _doConnect({ localPort, serverHost, clientPort, onConnected });
  }
  ...
  function _doConnect(opts) {
    ...
    const req = http.request({
      ...
      res.on('end',   () => {
        if (_active) console.log(`  ↩  :${opts.localPort} tunnel disconnected — reconnecting…`);
        _retry(opts);
      });
  ...
  function _retry(opts) {
    ...
    if (_attempt > 1) {
      console.log(`  ↻  :${opts.localPort} retry #${_attempt} in ${Math.round(delay / 1000)}s…`);
    }
  ...
  function _forward(msg, { localPort, serverHost, clientPort }) {
    const body = Buffer.from(msg.body || '', 'base64');
    const hdrs = { ...msg.headers, host: `localhost:${localPort}` };
    delete hdrs['content-length'];
    if (body.length) hdrs['content-length'] = body.length;

    const pr = http.request({
      hostname: 'localhost', port: localPort,
      ...
      proxyRes.on('end', () => {
        ...
        console.log(`  ${icon} :${localPort} ${msg.method.padEnd(6)} ${msg.url.padEnd(36)} ${proxyRes.statusCode}`);
      });
    });
    ...
    pr.on('error', err => {
      ...
      console.error(`  ✗ :${localPort} ${msg.method} ${msg.url} → ${err.message}`);
