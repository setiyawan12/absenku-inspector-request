 ...
 */
function createTunnelServer(localPort) {
  return http.createServer(async (req, res) => {
    const tunnel    = state.tunnels.get(localPort) || {};
    const requestId = uid();
    const t0        = Date.now();
    ...
    // ── 1. Mock check (works even without a client) ───────────────────────────
    ...
    if (mock) {
      ...
      const mockHeaders = Object.assign(
        ...
        mock.headers,
        { 'x-mocked-by': 'absenku Inspector request', 'content-length': String(mockBodyBuf.length) },
      );
    ...
    if (!tunnel.clientSSE || tunnel.clientSSE.writableEnded) {
      ...
      res.end(`<!DOCTYPE html><html><body style="font-family:monospace;background:#0a0b14;color:#e0e0f0;display:flex;align-items:center;justify-content:center;height:100vh;margin:0">
        <div style="text-align:center">
          ...
          <p style="color:#888">Run: <code style="background:#1a1a2e;padding:4px 10px;border-radius:6px;color:#818cf8">absenku http ${localPort}</code></p>
        </div>
      </body></html>`);
      return;
    }
    ...
    const _injPort   = state.headerInjections.get(localPort) || {};
    ...
    tunnel.pending.set(requestId, (msg) => {
      ...
      entry.resBodyB64 = _bin ? rb.toString('base64') : null;
      entry.ms         = Date.now() - t0;
    ...
    // Timeout after 30 s
    setTimeout(() => {
      if (!done && tunnel.pending.has(requestId)) {
        done = true;
 ...
 */
function attachWebSocketProxy(server, localPort) {
...
function _log(entry) {
  ...
  if (state.requestLog.length > MAX_LOG) state.requestLog.pop();
